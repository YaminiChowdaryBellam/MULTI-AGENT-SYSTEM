...
>
> Live deployment: Azure Container Apps. First attempt used `az containerapp up --source .`, which builds via Azure Container Registry Tasks (cloud build, no local Docker needed) — but ACR Tasks is blocked by default on brand-new Azure subscriptions ("TasksOperationsNotAllowed"), a real, known restriction, not something fixable in code. Fell back to starting Docker Desktop locally, `docker build --platform linux/amd64` (Container Apps runs amd64; this machine is arm64), `docker push` to the already-created ACR, then `az containerapp create` referencing that image directly with `--secrets`/`secretref` for `GROQ_API_KEY`/`TAVILY_API_KEY` (not plain env vars) — reusing the environment and registry that had already provisioned successfully before the Tasks step failed. The deployed image uses `requirements-docker.txt`, a slim runtime-only subset of `requirements.txt` that excludes `sentence-transformers`/`scikit-learn` (Phase 5.4's embedding router — eval/benchmark-only, never imported by `api.py`) and `pytest`/`ruff` (dev-only): 190MB built. `min-replicas=0` (scale-to-zero) keeps it comfortably inside the free tier.
>
> Pushing to the real repo (rather than local-only testing) immediately surfaced two more reliability gaps `update-eval-baseline.yml`'s heavier load (50 routing + 10 judge calls in quick succession, more Groq pressure than any local run in this session) exposed: `call_groq` only retried `RateLimitError`, so a plain `groq.APIConnectionError` — seen only in GitHub Actions' network environment, never locally — crashed the run outright; and even after that fix, the 1s/2s/4s backoff schedule was consistently shorter than Groq's actual reported TPM-reset window (5-12s in every observed 429 response), so retries exhausted before ever actually waiting long enough. Fixed by broadening the caught exception set (`RateLimitError`, `APIConnectionError`, `APITimeoutError`, `InternalServerError` — deliberately not `BadRequestError`/`AuthenticationError`, where retrying an identical request can't help) and raising the backoff base to 8s (8s/16s/32s, 4 attempts).
>
> With both fixed, `update-eval-baseline.yml` ran to completion for the first time — but still exited 1: routing accuracy held flat (70%) and F1 improved, but faithfulness/citation_coverage/review_rate each shifted by about 10% on the 10-case judge sample. At n=10, one case flipping outcome *is* a 10% swing — this reads as ordinary LLM sampling noise, not a real regression (nothing in the Phase 6 changes touches synthesis, citation enforcement, or the review gate), and the 0.02 regression tolerance is tighter than that sample size's noise floor can support. The workflow correctly refused to commit a baseline it flagged as regressed, which is the safe default; raising `--judge-limit` (more samples, smaller noise) or loosening the tolerance for judge-derived metrics specifically would be the honest fix, left as a follow-up rather than done reflexively to make a run go green.
...
| Cloud deploy | Azure Container Apps | Consumption-based free tier, scale-to-zero; local `docker build`/`push` to ACR (cloud build via ACR Tasks is blocked on new subscriptions) |
